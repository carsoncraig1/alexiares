...
const handleCVR = async (req, res) => {
    ...
    console.log(`Conversion data received: s1=${s1}, s4=${s4}, ttclid=${ttclid}, payout=${payout}, tid=${tid}`);
    try {
        if (s4) {
            const postbackUrl = `https://klcxb6.mcattr.com/conv?clid=${s4}&payout=${payout}&txid=${tid}`;
            try {
                const response = await axios.get(postbackUrl);
                console.log(`Postback sent successfully: ${postbackUrl}`);
                console.log(`Postback response: ${response.status} ${response.statusText}`);
            } catch (postbackError) {
                console.error('Error sending postback:', postbackError);
            }
        } else {
            console.log('No s4 value present, skipping postback');
        }
        res.sendStatus(200);
    } catch (error) {
